...
class POS(object):
    def __init__(self, file):
        self.poscar = open(file)
        
    def read_pos(self):
        p_dict = {}
        p_dict["name"] = self.poscar.readline()
        ...
        print p_dict["natoms"]
        selective = self.poscar.readline()
        if selective.split()[0][0] in ['s','S']: 
            p_dict["selective"] = True
            p_dict["csystem"] = self.poscar.readline().split()[0][0]
        else: 
    ...
    def write_sgroup(self, pos):
        ...
        beta = np.arccos(np.dot(pos["vlatt_3"]*pos["scale"],pos["vlatt_1"]*pos["scale"])/(b*c))*180./np.pi
        gamma = np.arccos(np.dot(pos["vlatt_1"]*pos["scale"],pos["vlatt_2"]*pos["scale"])/(b*c))*180./np.pi
...
if __name__ == "__main__": POS('POSCAR').read_pos()