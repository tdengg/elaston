...
class SETUP(object):
    def __init__(self, conserveV = False):
        ...
        for i in Lag_strain_list:
            ...
            os.mkdir(Dstn)
            os.chdir(Dstn)
            if i == '08' and conserveV:
                Ls_str[i] = '(  eta(eta/2+1),  eta(eta/2-1),  eta^2/(1-eta^2)(1+eta^2/(2(1-eta^2))),  0.0,  0.0,  0.0)'
            ...
            for s in range(-ptn, ptn+1):
                ...
                if i == '08' and conserveV:
                    Lv[0] = r*(r/2.+1.)
                    Lv[1] = r*(r/2.-1.)
                    Lv[2] = r**2./(1.-r**2.)*(1+0.5*r**2./(1.-r**2.))
        ...
        #--------------------------------------------------------------------------------------------------
                ...
                print eps_matrix
                print 'Det(d) = ', np.linalg.det(def_matrix)
                cont2 = cont2 + 1
                if (cont2 < 10):
