        ...
        if (os.path.exists(Dstn_num)): 
            ...
            if (os.path.exists(energy_output)):
                ...
                if (cod == 'vasp'):
                    ...
                    velem = vaspout.xpath("//i[@name = 'volume'][last()]")
                    
                    volume = float(velem[0].text)
...
for i in range(1, ECs+1):
    ...
    if (os.path.exists('Grace.par') == False):
        ...
        elastichome =  sys.argv[0].rstrip('ElaStic_Analyze.py')
        os.system("cp -f " + elastichome + "Grace.par .")
